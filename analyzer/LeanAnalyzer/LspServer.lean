...
def convertGoalInfo (g : Paperproof.Services.GoalInfo) : LeanAnalyzer.GoalInfo :=
  { type := g.type
    username := filterNameOpt g.username
    id := g.id.name.toString }

def convertHypothesis (h : Paperproof.Services.Hypothesis) : LeanAnalyzer.HypothesisInfo :=
...
def buildProofDagFromSteps (steps : List Paperproof.Services.ProofStep) (_line _col : Nat) : ProofDag :=
...
def handleGetProofDag (params : GetProofDagParams) : RequestM (RequestTask GetProofDagResult) := do
  ...
  RequestM.withWaitFindSnapAtPos params.position fun snap => do
    ...
    | "tree" =>
      match ← Paperproof.Services.BetterParser snap.infoTree with
      | some result =>
      ...
      | none =>
        ...
        return { proofDag := {} }
    | "single_tactic" =>
      match Paperproof.Services.goalsAt? snap.infoTree text hoverPos with
      | r :: _ =>
        let result ← Paperproof.Services.parseTacticInfo snap.infoTree r.ctxInfo (.ofTacticInfo r.tacticInfo) [] ∅ true
